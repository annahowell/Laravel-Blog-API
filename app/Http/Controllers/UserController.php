...
class UserController extends Controller
{
    ...
    public function login(UserLoginRequest $request): JsonResponse
    {
        ...
        if (false === Auth::attempt($credentials) || false === $request->user()->enabled) {
    ...
    public function update(UserRoleRequest $request, User $user): JsonResponse
    {
        ...
        if (true === Auth::user()->hasRole('admin') && $request->roles) {
            $user->syncRoles($request->roles);
        }

        if (true === $request['password']) {
            $request['password'] = Hash::make($request->password);
        }

        if ($request['enabled'] !== null && false === $request['enabled']) {
